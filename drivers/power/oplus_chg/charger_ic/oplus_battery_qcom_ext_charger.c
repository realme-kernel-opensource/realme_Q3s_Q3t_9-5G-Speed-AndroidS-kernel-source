...
void opchg_usbin_valid_irq_handler(void)
{
	...
	if(oplus_gauge_check_chip_is_null() || oplus_warp_check_chip_is_null() || oplus_chg_check_chip_is_null())
	{
		pr_err("[do_chrdet_int_task] warp || gauge || charger not ready, will do after bettery init.\n");
		return ;
	}
